...
def CF(V_i, B_i):
    ...
    for j in range(0, 16):
        W.append(0)
        unpack_list = struct.unpack(">I".encode(), B_i[j*4:(j+1)*4])
        W[j] = unpack_list[0]
...
def hash_msg(msg):
    ...
    for i in range(reserve1, range_end):
        msg = msg + chr(0x00).encode()

    bit_length = (len1) * 8
    bit_length_string = struct.pack(">Q", bit_length)
    msg = msg + str(bit_length_string).encode()
...
# print ("result: ",which)
# print ("abcd" * 16)
# y = hash_msg(b"abcd" * 16)
# print ("result: ",y)
# out_hex(y)

def sm3(text):
    return out_hex(hash_msg( text.encode(encoding="utf-8")))
